...
SELECT location, 
       ...
       total_cases, 
       population, 
       (CAST(total_cases AS FLOAT) / CAST(population AS FLOAT)) * 100 AS affected_perc
FROM CovidDeaths
--WHERE location like '%states%';
Where continent IS NOT NULL
...
SELECT Location, 
       ...
       population, 
       (CAST(MAX(total_cases) AS FLOAT) / CAST(population AS FLOAT)) * 100 AS affected_perc
FROM CovidDeaths
--WHERE location like '%states%'
Where continent IS NOT NULL;
